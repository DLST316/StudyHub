package dev.kang.studyhub.study.service;
 ...
 * 스터디에 승인된 사용자, 스터디 개설자, 관리자만 댓글을 작성할 수 있도록 제한합니다.
 */
@Service
...
public class StudyCommentService {
    ...
    /**
     * 댓글 작성
     * 스터디에 승인된 사용자, 스터디 개설자, 관리자만 댓글을 작성할 수 있습니다.
     */
    @Transactional
    public StudyComment createComment(String content, User user, Study study) {
        ...
        // 스터디 개설자이거나 어드민은 항상 댓글 작성 가능
        if (study.isLeader(user) || "ADMIN".equals(user.getRole())) {
    ...
    public StudyComment updateComment(Long commentId, String content, User user) {
        ...
        if (!comment.isAuthor(user) && !"ADMIN".equals(user.getRole())) {
            throw new IllegalStateException("댓글 작성자 또는 관리자만 수정할 수 있습니다.");
        }
    ...
    public void deleteComment(Long commentId, User user) {
        ...
        // 댓글 작성자이거나 스터디 개설자이거나 어드민인 경우에만 삭제 가능
        if (!comment.isAuthor(user) && !comment.getStudy().isLeader(user) && !"ADMIN".equals(user.getRole())) {
            throw new IllegalStateException("댓글 작성자, 스터디 개설자 또는 관리자만 삭제할 수 있습니다.");
        }
